...
**Clickable linked vertex** (preferred for source/code links in draw.io UI):
```xml
<mxCell id="linked-id"
        link="https://example.com/file.py#L10-L20"
        value="&lt;a href=&quot;https://example.com/file.py#L10-L20&quot;&gt;Linked label&lt;br&gt;second line&lt;/a&gt;"
        style="rounded=1;whiteSpace=wrap;html=1;fillColor=#dae8fc;strokeColor=#6c8ebf;"
        vertex="1" parent="1">
...
- For clickable code/source links in draw.io, prefer `mxCell link="..."` **plus** an embedded HTML anchor in `value`; keep both URLs identical
- When the whole label is clickable, use `<br>` inside the anchor instead of `&#xa;`
...
2. **Run the validator** (optional but recommended):
   ```bash
   python skills/draw-io-diagram-generator/scripts/validate-drawio.py <path-to-file.drawio>
   ```
...
```bash
python skills/draw-io-diagram-generator/scripts/add-shape.py docs/arch.drawio "New Service" 700 380
```
...
All companion files are in `skills/draw-io-diagram-generator/`:

...
2. **Select** the matching template from `skills/draw-io-diagram-generator/assets/templates/` and adapt it, or start from the minimal skeleton
...
- For links that should open reliably in draw.io web and desktop, use **both**:
  - `link="https://..."` on the `mxCell`
  - an embedded HTML anchor in `value` with `html=1`
- Keep the outer `link` URL and the inner `<a href="...">` URL **identical**
- When the label is fully linked, use `<br>` inside the anchor instead of `&#xa;`

```xml
<mxCell
  id="linked-node"
  link="https://example.com/file.py#L10-L20"
  value="&lt;a href=&quot;https://example.com/file.py#L10-L20&quot;&gt;Detect head&lt;br&gt;export = False&lt;/a&gt;"
...
| File | Use For |
|---|---|
| `skills/draw-io-diagram-generator/SKILL.md` | Full agent workflow, recipes, troubleshooting |
